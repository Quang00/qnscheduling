...
class TestGenerateWaxmanGraph:
    def test_waxman_basic(self, waxman_result):
        nodes, edges, fidelities, rates, avg_deg, diameter = waxman_result
...
def test_fat_tree_counts(k, expected_nodes, expected_qpus):
    nodes, _, _, _, qpus, _ = fat_tree(k=k)
...
class TestFatTree:
    def test_returns_valid_tuple(self, fat_tree_k4):
        nodes, edges, fidelities, rates, qpus, diameter = fat_tree_k4
        assert isinstance(nodes, list)
        assert isinstance(edges, list)
        assert isinstance(fidelities, dict)
        assert isinstance(rates, dict)
    ...
    def test_dist_fidelity(self):
        _, _, fid_close, _, _, _ = fat_tree(k=4, qpu_edge_dist=0.1)
        _, _, fid_far, _, _, _ = fat_tree(k=4, qpu_edge_dist=10.0)
